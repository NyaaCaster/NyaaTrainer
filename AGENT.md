...
- **③ 独立修改器**：格式逆向（`stub + PE 资源 ARCHIVE/DECOMPRESSOR`、`.cepack`）／**无 GUI 全自动生成**／11 个必打包文件／小面板 UI（含**四个"看着能用其实不能用"的 API**、**任务栏显示**、`getControl(i)` 枚举控件）／**§4.5.1 锁定功能只能有一个入口（模板约定）**／退出行为与残留进程／**DPI 陷阱与尺寸 API 不可靠的实测数据**／28 条坑位／**§7.1 生成侧自检**（含 Lua 作用域自查）
